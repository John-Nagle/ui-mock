...
impl UiInfo {
    ...
    pub fn change_mode(&mut self, new_mode: SystemMode) {
        ...
        println!("System state change: {:?} -> {:?}", self.system_mode, new_mode); // ***TEMP***
...
pub fn pick_replay_file_async(state: &mut CommonState, window: &winit::window::Window) {
    ...
    execute(async move {
        ...
        let _ = CommonState::send_gui_event_on_channel(&channel, Box::new(GuiEvent::OpenReplay(replay_path_opt))); // if we can't send, we must be shutting down
    });
}
